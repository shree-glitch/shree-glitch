## Hi there 👋

# _**"Without Data you are just another person with an opinion!"**_  ### _**- W. Edwards Deming, Data Scientist**_

### About Me 🚀

- 💡 I am Shreeraj Kulkarni, a Data Scientist with 4+ years of Industry Experience.

- 🛫 I am Currently working as Data Scientist 2 with Pratt and Whitney and have worked as Thermal Engineer with Rolls Royce Aerospace previously. 

- 🎓 I have Master of Business Administration with specialization in Information Technology and Systems Management and Bachelor of Engineering in Mechanical Engineering.

- 👨‍💻 I specialize in _data analytics_, _machine learning_ and _end-to-end ML model deployment_. I have worked extensively as **AI/ML engineer** for _Generative AI_ application development. I have built RAG application, have finetuned Large Language Models and also developed Agentic AI based RAG tool.

- ♔♚⛰️🎶 Apart from being a Data Scientist, I like to read, go on treks and play a lot of Chess!

### 🏅 Certifications Bucket

1. Generative AI with LLMs - Deeplearing.AI
     [Generative AI with LLMs](https://coursera.org/verif%20y/B2DUNE75JU88?trk=public_profile_see-credential)
     
2. Amazon Certified Cloud Practitioner.
     [Amazon Certified Cloud Practitioner](https://aws.amazon.com/verification?trk=public_profile_see-credential)
     
3. Microsoft Certified Power BI Data Analyst.
     [Microsoft Certified Power BI Data Analyst](https://learn.microsoft.com/api/credentials/share/en-us/ShreerajKulkarni/11B91DEF49327391?sharingId=64B70DE6BFA1D9F1&trk=public_profile_see-credential)
     
4. Unleashing LLMs: Creating RAG applications by Training LLMs, Finetuning and Evaluating - Data Hack Summit 2024.

### 📱 Socials 

[![Github](https://img.shields.io/badge/GitHub-000000?style=for-the-badge&logo=GitHub&logoColor=white)](https://github.com/shree-glitch)

[![LinkedIn](https://img.shields.io/badge/LinkedIn-0077B5?style=for-the-badge&logo=LinkedIn&logoColor=white)](https://linkedin.com/in/shreeraj-kulkarni)
